...
import { Clock, Moon, Plus, Power, Settings, ShieldAlert } from "lucide-react";
...
import { formatDuration, isScheduleActive, parseClockTime } from "../shared/time";
...
function PopupApp(): JSX.Element {
  ...
  const blockedToday = events.filter((event) => event.type === "blocked" && isToday(event.createdAt)).length;
  const unlockedToday = events.filter((event) => event.type === "unlocked" && isToday(event.createdAt)).length;
  ...
  async function pauseFifteenMinutes(): Promise<void> {
    ...
    await recordGuardEvent({ type: "paused", host: "*" });
    setSettings(next);
  }
  ...
  return (
    <main className="w-80 overflow-hidden border border-slate-700 bg-slate-900 text-slate-100 shadow-night">
      ...
      <section className="px-5 py-6 text-center">
        ...
        <h1 className="text-lg font-semibold text-white">{isActive ? "晚安模式已开启" : "晚安模式未开启"}</h1>
        <p className="mt-1 text-sm text-slate-400">{settings ? scheduleCopy(settings) : "正在读取计划..."}</p>

        <div className="mt-5 grid grid-cols-2 gap-3">
          <StatCard label="今日阻断" value={blockedToday} />
          <StatCard label="临时解锁" value={unlockedToday} />
        </div>
      </section>
