...
import { evaluateReminder, formatDuration, isScheduleActive } from "../shared/time";
...
function PopupApp(): JSX.Element {
  ...
  const hasActiveGroup = settings?.ruleGroups.some((group) => group.enabled && isScheduleActive(group.schedule));
  const hasReminder = settings?.ruleGroups.some((group) =>
    evaluateReminder(group, settings.remindedSessionIds).shouldRemind
  );
  const stats = settings ? buildStatsSummary(events, settings.ruleGroups[0]?.schedule ?? { enabled: false, startTime: "00:00", endTime: "00:00", days: [] }) : undefined;
  const targetGroup = settings?.ruleGroups[0];
    ...
    <main className="w-80 overflow-hidden border border-slate-700 bg-slate-900 text-slate-100 shadow-night">
      ...
      <section className="px-5 py-6 text-center">
        <div className="mx-auto mb-3 flex h-16 w-16 items-center justify-center rounded-2xl border border-indigo-400/30 bg-indigo-500/10">
          {hasReminder ? <Bell className="h-8 w-8 text-amber-300" /> : <Clock className="h-8 w-8 text-indigo-300" />}
        </div>
        <h1 className="text-lg font-semibold text-white">
          {hasActiveGroup ? "限制规则正在生效" : hasReminder ? "限制规则即将开启" : "当前没有规则生效"}
        </h1>
        <p className="mt-1 text-sm text-slate-400">{settings ? statusCopy(settings) : "正在读取计划..."}</p>
      ...
      <section className="border-y border-slate-800 bg-slate-950/50 p-4">
        <div className="mb-3 flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            {decision?.ruleGroupName && <p className="mt-1 truncate text-xs text-indigo-300">{decision.ruleGroupName}</p>}
          </div>
      ...
      <footer className="flex items-center justify-between gap-3 p-4">
        ...
        <a
          ...
          href={getExtensionUrl(`block.html?site=${encodeURIComponent(host)}${decision?.ruleGroupId ? `&group=${encodeURIComponent(decision.ruleGroupId)}` : ""}`)}
          target="_blank"
          rel="noreferrer"
        >
...
function statusCopy(settings: AppSettings): string {
  ...
  const active = enabled.filter((group) => isScheduleActive(group.schedule));
  if (active.length > 0) {
    return `${active.map((group) => group.name).join("、")} 正在生效`;
  }
