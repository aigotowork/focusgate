...
公开网站会读取浏览器的 `navigator.languages` 或 `navigator.language`，用于在默认中文入口上选择中文、英文或法文页面。语言切换器可能会在本地浏览器的 `localStorage` 中保存一个语言代码，以便下次打开默认入口时沿用用户选择。

这个偏好只用于公开网站的语言跳转，不会被本静态网站发送到开发者服务器，也不会改变扩展本身的数据处理方式。
